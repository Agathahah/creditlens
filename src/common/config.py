...
class Settings(BaseSettings):
    ...
    # Environment
    ENV: str = Field(
        default="development", description="Current environment (development, production, test)"
    )
    ...
    FRED_API_KEY: str | None = Field(
        default=None, description="API Key for Federal Reserve Economic Data (FRED)"
    )
    OPENAI_API_KEY: str | None = Field(default=None, description="API Key for OpenAI (Embeddings)")
    ANTHROPIC_API_KEY: str | None = Field(default=None, description="API Key for Anthropic Claude")

    # MLflow
    MLFLOW_URL: str = Field(
        default="http://localhost:5000", description="MLflow tracking server URL"
    )

    # Grafana
    GRAFANA_PASSWORD: str = Field(
        default="admin", description="Admin password for Grafana dashboard"
    )
